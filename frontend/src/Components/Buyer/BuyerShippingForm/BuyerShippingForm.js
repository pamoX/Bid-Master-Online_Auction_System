...
function BuyerShippingForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      localStorage.setItem(`buyer_shipping_${formData.itemid}`, JSON.stringify(formData));
      
      // Check if seller data exists
      const sellerData = localStorage.getItem(`seller_shipping_${formData.itemid}`);
      
      if (sellerData) {
        // If seller data exists, combine and submit
        const sellerShippingData = JSON.parse(sellerData);
        const combinedData = {
          ...sellerShippingData,
          ...formData,
          status: 'Pending',
        ...
        // If no seller data, just save buyer data and show message
        toast.info('Buyer details saved. Waiting for seller details.');
        navigate('/orders');
      }
    } catch (error) {
